...
import { getUserReservations, getEnrolledCourses, getUpcomingSessions } from '../api';
        ...
        const fetchData = async () => {
            ...
            if (storedUser) {
                ...
                try {
                    ...
                    setBookings((resReservations.data || []).map(item => {
                        const data = item.attributes || item;
                        const space = data.coworking_space?.data?.attributes || data.coworking_space || {};
                        return {
                            id: item.id,
                            spaceName: space.name || 'Espace inconnu',
                            date: new Date(data.date).toLocaleDateString('fr-FR', { weekday: 'long', day: 'numeric', month: 'short' }),
                            time: data.time_slot || 'Non spécifié'
                        };
                    }));

                    setCourses((resCourses.data || []).map(item => {
                        const data = item.attributes || item;
                        return {
                            id: item.id,
                            title: data.title,
                    ...
                    setSessions((resSessions.data || []).map(item => {
                        const data = item.attributes || item;
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
                    <div>
                        <h1 className="text-3xl font-extrabold text-gray-900 tracking-tight">
                            Bonjour, <span className="text-blue-600">{user.fullname || user.username}</span> ! 👋
                        </h1>
                        <p className="text-gray-600 mt-1">Heureux de vous revoir sur votre espace d'apprentissage.</p>
                    </div>
                    ...
                    <div className="lg:col-span-1">
                        ...
                        <div className="mt-8 p-6 bg-gradient-to-br from-indigo-600 to-purple-700 rounded-2xl text-white shadow-xl">
                            ...
                            <p className="text-indigo-100 text-sm leading-relaxed">
                                Saviez-vous que vous pouvez modifier vos préférences de notification par email directement depuis votre profil ?
                            </p>
                            <button
                                onClick={() => navigate('/profile')}
