...
import { getSubscriptionPlans, subscribeToPlan, getMySubscription } from '../services/subscriptionService';
...
const PLAN_COLORS = {
    basic: { gradient: 'from-slate-700 to-slate-900', badge: 'bg-slate-100 text-slate-700', btn: 'bg-slate-800 hover:bg-slate-950', ring: 'ring-slate-300' },
    premium: { gradient: 'from-blue-600 to-indigo-800', badge: 'bg-blue-100 text-blue-700', btn: 'bg-blue-600 hover:bg-blue-700', ring: 'ring-blue-400' },
    enterprise: { gradient: 'from-amber-500 to-orange-700', badge: 'bg-amber-100 text-amber-700', btn: 'bg-amber-600 hover:bg-amber-700', ring: 'ring-amber-400' },
};
...
const SubscriptionPlans = () => {
    ...
    useEffect(() => {
        const load = async () => {
            try {
                ...
                if (apiPlans.length > 0) {
                    ...
                    const mapped = apiPlans.map((p) => {
                        ...
                        return {
                            ...
                            features: Array.isArray(a.features) ? a.features : (a.features ? Object.values(a.features) : []),
                        };
                    });
                    setPlans(mapped);
                }
                ...
                console.warn('[Subscription] API not reachable, using static plans:', err.message);
                // Keep FALLBACK_PLANS
            } finally {
                setLoading(false);
            }
    ...
    const getPrice = (plan) => {
        ...
        return billingCycle === 'yearly' ? Math.round(base * 12 * 0.8) : Math.round(base);
    };
    ...
    const handleSubscribe = async (plan) => {
        ...
        if (!window.confirm(`Confirmer l'abonnement au plan ${plan.name} (${billingCycle === 'yearly' ? 'Annuel -20%' : 'Mensuel'}) ?`)) return;

        try {
            setSubscribing(plan.id);
            if (plan.documentId || typeof plan.id === 'number') {
                await subscribeToPlan({ user: user.id, plan: plan.documentId || plan.id, billing_cycle: billingCycle });
                alert(`🎉 Félicitations ! Vous êtes maintenant abonné au plan ${plan.name}.`);
                navigate('/professional/subscription');
            } else {
                // Fallback static mode
                alert(`✅ Demande enregistrée pour le plan ${plan.name}. Contactez-nous pour finaliser votre abonnement.`);
            }
        } catch (err) {
        ...
        <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 font-sans">
            ...
            <div className="max-w-6xl mx-auto px-4 pt-16 pb-8 text-center">
                ...
                <p className="text-slate-500 text-lg max-w-xl mx-auto mb-8">
                    Accédez régulièrement aux meilleurs espaces de coworking de Tunis avec nos formules flexibles.
                </p>
                ...
                <div className="inline-flex bg-white/80 backdrop-blur border border-slate-200 rounded-2xl p-1.5 shadow-lg">
                    ...
                    <button onClick={() => setBillingCycle('yearly')}
                        ...
                        Annuel
                        <span className="text-[9px] bg-emerald-500 text-white px-1.5 py-0.5 rounded-full font-black">-20%</span>
                    </button>
                </div>
            </div>
            ...
            {mySubscription && (
                <div className="max-w-xl mx-auto px-4 mb-8">
                    <div className="p-5 bg-emerald-50 border border-emerald-200 rounded-2xl flex items-center gap-4">
                        ...
                        <div>
                            <p className="text-sm font-black text-emerald-800">Abonnement actif</p>
                            <p className="text-xs text-emerald-600">
                                Plan {(mySubscription.attributes || mySubscription).plan?.data?.attributes?.name || (mySubscription.attributes || mySubscription)?.plan?.name} ·
                                Expire le {new Date((mySubscription.attributes || mySubscription).end_date).toLocaleDateString('fr-FR')}
                            </p>
                        </div>
                        <button onClick={() => navigate('/professional/subscription')}
            ...
            <div className="max-w-6xl mx-auto px-4 pb-12">
                        ...
                        return (
                            <div key={plan.id}
                                ...
                                <div className={`bg-gradient-to-br ${colors.gradient} p-8`}>
                                    <span className={`inline-block px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest mb-4 ${colors.badge}`}>
                                        {plan.type}
                                    </span>
                                    <h3 className="text-2xl font-black text-white mb-1">{plan.name}</h3>
                                    <p className="text-white/70 text-xs mb-6">{plan.description}</p>
                                    <div className="flex items-baseline gap-1">
                                        <span className="text-5xl font-black text-white">{getPrice(plan)}</span>
                                        <span className="text-white/70 text-sm font-bold">DT/{billingCycle === 'yearly' ? 'an' : 'mois'}</span>
                                    </div>
                                    {billingCycle === 'yearly' && (
                                ...
                                <div className="bg-white p-8">
                                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4">
                                        {plan.max_credits >= 9999 ? '∞ crédits illimités' : `${plan.max_credits} crédits/mois`}
                                    </p>
                                    <ul className="space-y-3 mb-8 min-h-[200px]">
                                        {plan.features.map((f, i) => (
                                            <li key={i} className="flex items-center gap-3 text-sm text-slate-600">
                                                <span className="w-5 h-5 bg-emerald-50 rounded-full flex items-center justify-center text-emerald-500 text-xs flex-shrink-0">✓</span>
                                                {f}
                                            </li>
                                        ))}
                                    </ul>
                                    ...
                                    <button
                                        onClick={() => isCurrent ? navigate('/professional/subscription') : handleSubscribe(plan)}
                                        ...
                                        {subscribing === plan.id ? 'Traitement...' : isCurrent ? 'Plan Actuel ✓' : 'Souscrire maintenant'}
                                    </button>
                                </div>
                            </div>
                        );
                ...
                <div className="mt-16">
                    <button
                        ...
                        <span className="font-black text-slate-800">📊 Tableau comparatif complet</span>
                        <span className="text-slate-400 font-black">{showComparison ? '▲ Masquer' : '▼ Afficher'}</span>
                    </button>
                            ...
                            <table className="w-full text-sm">
                                <thead>
                                    <tr className="border-b border-slate-100">
                                        <th className="px-8 py-5 text-left text-[10px] font-black text-slate-400 uppercase tracking-widest w-1/3">Fonctionnalité</th>
                                        {plans.map((p) => (
                                            <th key={p.id} className="px-6 py-5 text-center">
                                                <span className={`inline-block px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest ${(PLAN_COLORS[p.type] || PLAN_COLORS.basic).badge}`}>
                                                    {p.name}
                                                </span>
                                            </th>
                                        ))}
                                ...
                                <tbody className="divide-y divide-slate-50">
                                    {COMPARISON_FEATURES.map((row, i) => (
                                        <tr key={i} className={i % 2 === 0 ? 'bg-slate-50/40' : ''}>
                                            <td className="px-8 py-4 text-slate-600 font-semibold">{row.label}</td>
                ...
                <p className="text-center text-xs text-slate-400 mt-10 font-medium">
                    * Tous les plans incluent l'accès au WiFi haut débit, aux espaces communs et au support de base.
                </p>
            </div>
        </div>
    );
