...
import LiveSessionModal from "../../components/dashboard/LiveSessionModal";
import MyGroupsWidget from "../../components/dashboard/MyGroupsWidget";
import StudentGroupModal from "../../components/dashboard/StudentGroupModal";
import {
  getMySubscription,
...
const TrainerDashboard = ({ activeTab = "dashboard" }) => {
  ...
  const fetchData = async () => {
    ...
    if (storedUser) {
      ...
      try {
        const [resCourses, resSessions, resStudents, subData, resGroups] =
        ...
        if (resCourses.status === "fulfilled") {
          ...
          const coursesWithCounts = await Promise.all(
            coursesData.map(async (item) => {
              ...
              const coverRaw = attrs.cover?.data || attrs.cover;
              
              // Resolve category name from various possible structures
              const categoryName = 
                attrs.category_rel?.data?.attributes?.name || 
                attrs.category_rel?.name || 
                attrs.category || 
                "Général";

              // Resolve documents list
              const docsRaw = attrs.documents?.data || attrs.documents || [];
        ...
        if (resGroups.status === "fulfilled") {
          setGroups(resGroups.value.data || []);
        }
      ...
      <div className="mb-8 flex flex-col md:flex-row justify-between items-end gap-6">
        ...
        <div className="flex bg-slate-100 p-1.5 rounded-2xl">
          <button 
            ...
            className={`px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === "dashboard" ? "bg-white text-blue-600 shadow-xl" : "text-slate-400 hover:text-slate-600"}`}
          >
            📊 Dash
          </button>
          <button 
            onClick={() => navigate("/trainer/groups")}
            className={`px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === "groups" ? "bg-white text-blue-600 shadow-xl" : "text-slate-400 hover:text-slate-600"}`}
          >
            📁 Groupes
          </button>
        </div>
        <div className="flex gap-4">
          <button
            ...
            className="px-6 py-3 bg-white text-blue-600 border-2 border-blue-600 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-blue-50 transition-all shadow-lg"
          >
            📅 Planifier un Live
          </button>
          <button
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="bg-white/40 backdrop-blur-md p-6 rounded-[28px] border border-white/60">
            <MyGroupsWidget
              ...
              onSeeAll={() => navigate("/trainer/groups")}
              onManage={(group) => {
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-xs text-slate-500 font-medium">
            Créez des classes et organisez vos membres pour un meilleur suivi.
          </p>
        </div>
        <div className="flex gap-4">
          <button
            ...
            className="px-6 py-2 bg-slate-900 text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-black transition-all shadow-lg"
          >
            + Nouveau Groupe
          </button>
          <button
  ...
  const handleSaveGroup = async (groupData) => {
    try {
      setLoading(true);
      if (selectedGroup) {
        await updateGroup(selectedGroup.documentId || selectedGroup.id, groupData);
