...
import { OrbitControls, Environment, Grid, Center, useProgress, Html, useGLTF } from "@react-three/drei";
import {
    getModels,
    ...
    return (
        <Html center>
            <div className="flex flex-col items-center gap-3 text-white font-medium">
                ...
                <span className="text-xs opacity-70">Chargement... {progress.toFixed(0)}%</span>
            </div>
        </Html>
    );
}
...
const ModelManagement = () => {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            setMessage({ type: "error", text: `Erreur : ${error.response?.data?.error?.message || "Chargement impossible"}` });
        } finally {
            setLoading(false);
        }
    }, []);
    ...
    const handleUpload = async (file) => {
        ...
        if (!["glb", "gltf", "fbx"].includes(ext)) {
            setMessage({ type: "error", text: "Format non supporté (.glb, .gltf, .fbx uniquement)" });
            return;
        }
        ...
        try {
            ...
            setMessage({ type: "error", text: error.response?.data?.error?.message || "Échec de l'upload" });
        } finally {
            setUploading(false);
        }
    };
        ...
        <AdminLayout>
            <div className="max-w-6xl mx-auto animate-fade-in pb-20">
                <div className="flex justify-between items-center mb-8">
                    <div>
                        <h1 className="text-3xl font-extrabold text-slate-800 tracking-tight">Gestion des Modèles 3D</h1>
                        <p className="text-slate-500 mt-1">Gérez les plans architecturaux de vos espaces de coworking.</p>
                    </div>
                    {message.text && (
                        <div className={`px-4 py-2 rounded-lg text-sm font-medium animate-bounce 
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-1 border-r border-slate-200 pr-0 lg:pr-8">
                        <div className="bg-white/80 backdrop-blur-xl rounded-2xl p-6 shadow-xl shadow-slate-200/50 border border-white/60 sticky top-4">
                            ...
                            <div className="mb-4">
                                <label className="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Espace associé</label>
                                <select
                                    value={selectedSpace}
                                ...
                                >
                                    {spaces.map(s => (
                                        <option key={s.id} value={s.id}>{s.name || s.attributes.name}</option>
                                    ))}
                                </select>
                            </div>
                  ...
                  ${uploading ? "opacity-50 pointer-events-none" : "cursor-pointer"}`}
                            >
                                ...
                                />
                                <label htmlFor="file-upload" className="cursor-pointer w-full h-full absolute inset-0 z-0" />
                  ...
                  ${dragActive ? "bg-blue-500 text-white scale-110" : "bg-white text-slate-400 shadow-sm group-hover:scale-110 group-hover:bg-blue-50 group-hover:text-blue-500"}`}>
                                ...
                                <h4 className="font-bold text-slate-700 mb-1">Cliquer ou glisser ici</h4>
                                <p className="text-xs text-slate-400 px-4">Fichiers GLB, GLTF ou FBX supportés (Max 50 Mo)</p>
                            </div>
                        </div>
                    </div>
                                    ...
                                    <table className="w-full text-left border-separate border-spacing-y-3">
                                        ...
                                        <tbody>
                                            {models.map((model) => {
                                                ...
                                                return (
                                                    <tr key={model.id} className="bg-white group hover:shadow-lg hover:shadow-slate-200/40 transition-all duration-300">
                                                            ...
                                                            <div className="flex items-center gap-3">
                                                                ...
                                                                <div>
                                                                    ...
                                                                    <div className="text-xs text-slate-400 flex items-center gap-1">
                                                                        🏢 {m.coworking_space?.data?.attributes?.name || m.coworking_space?.name || "Non assigné"}
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        </td>
                                                        <td className="px-6 py-4 border-y border-slate-50 text-sm font-medium text-slate-500">
                                                            {((m.metadata?.size || 0) / (1024 * 1024)).toFixed(2)} Mo
                                                        </td>
            ...
            {previewModel && (
                <div className="fixed inset-0 z-[10001] flex items-center justify-center p-4">
                    <div className="absolute inset-0 bg-slate-900/40 backdrop-blur-md" onClick={() => setPreviewModel(null)}></div>
                    <div className="relative bg-white w-full max-w-5xl h-[80vh] rounded-[40px] shadow-2xl overflow-hidden animate-zoom-in flex flex-col border border-white/20">
                        ...
                        <div className="px-10 py-6 border-b border-slate-100 flex justify-between items-center bg-white/50 backdrop-blur-sm z-10">
                            <div>
                                <h3 className="text-2xl font-black text-slate-800 tracking-tight">{previewModel.attributes?.title || previewModel.title}</h3>
                                <p className="text-sm text-slate-400 font-medium">Aperçu 3D interactif</p>
                            </div>
                            <button
                        ...
                        <div className="flex-1 bg-[#0a0a0f] relative group">
                            <Canvas camera={{ position: [5, 5, 5], fov: 45 }} shadows>
                                ...
                                <Grid infiniteGrid fadeDistance={20} cellColor="#1a1a2e" sectionColor="#2a2a4e" />
