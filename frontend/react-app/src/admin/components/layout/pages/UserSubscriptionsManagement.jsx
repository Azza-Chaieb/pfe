...
const UserSubscriptionsManagement = () => {
    ...
    const fetchSubscriptions = async () => {
        try {
            ...
            let data = [];
            try {
                // Try the custom admin endpoint first (requires backend restart)
                const response = await api.get("/subscriptions/admin-all");
                data = response.data?.data || [];
            } catch (err) {
                // Fallback: use the core Strapi endpoint
    ...
    const getUserType = (user) => {
        ...
        if (t === "formateur" || t === "trainer") return "formateur";
        if (t === "etudiant" || t === "student") return "etudiant";
        return t;
    };
    ...
    const filtered = subscriptions.filter((s) => {
        ...
        const matchesType =
            userTypeFilter === "all" ||
            getUserType(user) === userTypeFilter;
        const matchesStatus =
                ...
                <div className="flex flex-wrap justify-between items-start mb-8 gap-4">
                    ...
                    <div className="flex flex-wrap gap-3">
                        ...
                        <div className="flex bg-white/50 p-1 rounded-xl border border-slate-200 shadow-sm">
                            {userTypeLabels.map((f) => (
                                <button
                                    key={f.value}
                                    onClick={() => setUserTypeFilter(f.value)}
                                    className={`px-4 py-2 rounded-lg text-xs font-bold transition-all uppercase tracking-widest ${userTypeFilter === f.value
                        ...
                        <div className="flex bg-white/50 p-1 rounded-xl border border-slate-200 shadow-sm">
                            {statusLabels.map((f) => (
                                <button
                                    ...
                                    className={`px-4 py-2 rounded-lg text-xs font-bold transition-all uppercase tracking-widest ${statusFilter === f.value
