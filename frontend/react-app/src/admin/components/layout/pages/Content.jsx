    ...
    const fetchContent = async () => {
      try {
        ...
        if (error.response && error.response.status === 403) {
          setError({
            type: 'permission',
            message: "Accès refusé (403). Vous n'avez pas la permission de voir ce contenu.",
            detail: "Connectez-vous au panneau d'administration Strapi (http://localhost:1337/admin) > Settings > Users & Permissions > Roles > Authenticated > Select 'Course' > Check 'find'."
          });
        } else {
          setError({ type: 'general', message: "Erreur lors du chargement du contenu." });
        }
      } finally {
        setLoading(false);
      }
    ...
    <AdminLayout>
      <div className="animate-fade-in">
        ...
        <div className="bg-white/80 backdrop-blur-xl rounded-3xl shadow-xl shadow-slate-200/50 border border-white/60 overflow-hidden">
          {loading ? (
            <div className="p-10 text-center text-slate-400 italic">Chargement du contenu...</div>
          ) : error ? (
            <div className="p-6 m-6 bg-red-50 border border-red-100 rounded-xl text-red-600">
              <div className="mt-4 p-4 bg-white/50 rounded-lg border border-red-200 text-sm">
                ...
                <ul className="list-disc pl-5 mt-1 space-y-1">
                  ...
                  <li>User ID: {JSON.parse(localStorage.getItem('user') || '{}').id || 'N/A'}</li>
                  <li>Email: {JSON.parse(localStorage.getItem('user') || '{}').email || 'N/A'}</li>
                  <li>Has Token: {localStorage.getItem('jwt') ? 'Yes' : 'No'}</li>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50/50 border-b border-slate-100">
                  <th className="p-5 text-xs font-bold text-slate-400 uppercase tracking-wider w-16">ID</th>
                  <th className="p-5 text-xs font-bold text-slate-400 uppercase tracking-wider">Titre</th>
                  <th className="p-5 text-xs font-bold text-slate-400 uppercase tracking-wider">Description</th>
                  <th className="p-5 text-xs font-bold text-slate-400 uppercase tracking-wider w-24">Actions</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                {content.map(item => (
                  <tr key={item.id} className="hover:bg-slate-50/80 transition-colors duration-200">
                    <td className="p-5 text-slate-400 font-mono text-sm">#{item.id}</td>
                    <td className="p-5">
                    ...
                    <td className="p-5 text-slate-500 max-w-md truncate">
                      {item.description || item.attributes?.description || 'Pas de description'}
                    </td>
                    <td className="p-5">
                      <button className="p-2 bg-slate-100 text-slate-400 rounded-lg cursor-not-allowed opacity-50" disabled>
                        ✏️
                      </button>
                    </td>
                  </tr>
            ...
            <div className="p-10 text-center text-slate-400 italic">
              <h3 className="text-xl font-bold text-slate-600 mb-2">Aucun cours trouvé (0 éléments)</h3>
              <p className="mb-6">La requête a réussi mais n'a retourné aucun résultat.</p>
              ...
              <div className="mx-auto max-w-md p-4 bg-slate-50 rounded-lg border border-slate-200 text-left text-sm">
                ...
                <ul className="list-disc pl-5 mt-1 space-y-1">
                  ...
                  <li>User ID: {JSON.parse(localStorage.getItem('user') || '{}').id || 'N/A'}</li>
                  <li>Has Token: {localStorage.getItem('jwt') ? 'Yes' : 'No'}</li>
                  <li>Data received: {Array.isArray(content) ? `Array(${content.length})` : typeof content}</li>
                </ul>
