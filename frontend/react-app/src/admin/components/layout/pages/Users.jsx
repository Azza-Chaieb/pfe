...
const Users = () => {
  ...
  const handleBulkAction = (actionType) => {
    ...
    const actionLabel = actionType === "block" ? "suspendre" : "débloquer";
    ...
    setConfirmConfig({
      ...
      type: actionType === "block" ? "danger" : "success",
      onConfirm: async () => {
        setLoading(true);
        try {
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute(
      "download",
      `sunspace_users_${new Date().toISOString().split("T")[0]}.csv`,
    );
  ...
  const statsData = [
    ...
    },
    {
      label: "Staff Admin",
      value: users.filter(
        (u) => u.user_type === "admin" || u.role?.name === "Admin",
      ).length,
      icon: "👑",
    ...
    return (
      <div className="fixed inset-0 z-[20000] flex items-center justify-center p-4">
        ...
        <div className="relative bg-white w-full max-w-md rounded-[2.5rem] shadow-2xl overflow-hidden animate-scale-up border border-white/20 p-10 text-center">
          <div
            className={`w-20 h-20 mx-auto rounded-3xl flex items-center justify-center text-3xl mb-6 shadow-xl ${confirmConfig.type === "danger" ? "bg-rose-50 text-rose-500 shadow-rose-100" : "bg-blue-50 text-blue-500 shadow-blue-100"}`}
          >
    ...
    return (
      <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4">
        ...
        <div className="relative bg-white w-full max-w-2xl rounded-[2.5rem] shadow-2xl overflow-hidden animate-scale-up border border-white/20">
          <div
            className={`p-10 text-white shadow-inner ${selectedUser.blocked ? "bg-gradient-to-br from-slate-700 to-slate-900" : "bg-gradient-to-br from-blue-600 to-indigo-800"}`}
          >
              ...
              <div className="flex items-center gap-6">
                ...
                <div>
                  ...
                  <div className="flex gap-2 mt-3">
                    ...
                    <span
                      className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider ${selectedUser.blocked ? "bg-red-500/30" : "bg-emerald-500/30"}`}
                    >
              ...
              <div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="bg-slate-50/50 p-4 rounded-2xl border border-slate-100">
                    ...
                    <p
                      className={`font-black flex items-center gap-2 ${selectedUser.confirmed ? "text-emerald-600" : "text-orange-500"}`}
                    >
            ...
            <div className="flex justify-end pt-8 border-t border-slate-100">
              <div className="flex gap-3">
                <button
                  ...
                  className={`px-8 py-3 font-black text-[10px] uppercase rounded-xl transition-all shadow-lg active:scale-95 ${selectedUser.blocked ? "bg-emerald-600 text-white shadow-emerald-200" : "bg-rose-600 text-white shadow-rose-200"}`}
                >
      ...
      <div className="animate-fade-in pb-20 max-w-[1600px] mx-auto">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
          ...
          <button
            ...
            className="group flex items-center gap-3 px-8 py-4 bg-white border border-slate-200 text-slate-700 font-black text-xs uppercase rounded-2xl hover:bg-slate-50 transition-all shadow-sm hover:shadow-md active:scale-95"
          >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
          {statsData.map((s, idx) => (
            <div
              key={idx}
              className="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-xl shadow-slate-200/20 flex items-center justify-between group hover:-translate-y-1 transition-all duration-500 relative overflow-hidden"
            >
              <div
                className={`absolute top-0 right-0 w-32 h-32 bg-gradient-to-br ${s.color} opacity-[0.03] rounded-full -mr-16 -mt-16`}
              ></div>
              ...
              <div
                className={`relative z-10 w-14 h-14 bg-gradient-to-br ${s.color} rounded-2xl flex items-center justify-center text-2xl shadow-lg ${s.shadow} text-white transition-transform group-hover:scale-110 duration-500`}
              >
                {s.icon}
              </div>
            </div>
        ...
        <div className="bg-white p-8 rounded-[2.5rem] mb-12 border border-slate-100 shadow-2xl shadow-slate-200/30">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-end">
            ...
            <div className="lg:col-span-1">
              ...
              >
                <span className="group-hover:rotate-180 transition-transform duration-500 text-lg">
                  🔄
                </span>
              </button>
            </div>
              ...
              <tbody className="divide-y divide-slate-50">
                {loading ? (
                  ...
                  paginatedUsers.map((user, idx) => (
                    <tr
                      key={user.id}
                      className={`group hover:bg-slate-50/80 transition-all duration-300 ${selectedUserIds.includes(user.id) ? "bg-blue-50/30" : ""}`}
                    >
                      <td className="p-8">
                        <input
                          type="checkbox"
                      ...
                      <td className="p-8">
                        <div className="flex items-center gap-5">
                          <div
                            className={`relative w-14 h-14 rounded-2xl flex items-center justify-center font-black text-xl transition-transform group-hover:scale-105 duration-300 ${user.blocked ? "bg-slate-100 text-slate-300" : "bg-gradient-to-br from-blue-600 to-indigo-700 text-white shadow-xl shadow-blue-500/20"}`}
                          >
                            {user.username?.charAt(0).toUpperCase() || "?"}
                            <div
                              className={`absolute -bottom-1 -right-1 w-4 h-4 rounded-full border-2 border-white ${user.blocked ? "bg-slate-300" : "bg-emerald-500"}`}
                            ></div>
                          </div>
                          <div>
                      ...
                      <td className="p-8">
                        <div className="inline-flex flex-col">
                          ...
                          <span
                            className={`mt-2 text-[9px] font-black uppercase tracking-wider ${user.user_type === "admin" || user.role?.name === "Admin" ? "text-amber-500" : "text-indigo-400"}`}
                          >
                            {user.user_type
                      ...
                      <td className="p-8">
                        <div
                          className={`inline-flex items-center gap-3 px-5 py-2 rounded-2xl border transition-all duration-300 ${user.blocked ? "bg-rose-50 border-rose-100" : "bg-emerald-50 border-emerald-100"}`}
                        >
                          <span
                            className={`w-2 h-2 rounded-full ${user.blocked ? "bg-rose-500" : "bg-emerald-500 animate-pulse"}`}
                          ></span>
                          <span
                            className={`text-[10px] font-black uppercase tracking-widest ${user.blocked ? "text-rose-500" : "text-emerald-600"}`}
                          >
                      ...
                      <td className="p-8 text-right">
                        <button
                          ...
                          className="px-10 py-4 bg-slate-900 text-white rounded-2xl text-[10px] font-black uppercase hover:bg-blue-600 transition-all active:scale-95 shadow-lg shadow-slate-200 hover:shadow-blue-500/20"
                        >
                          🎯 Gérer
                        </button>
                      </td>
                  ...
                  <tr>
                    <td colSpan="5" className="p-32 text-center">
                      <div className="opacity-30 flex flex-col items-center gap-4">
                        ...
                        <p className="font-black text-slate-400 uppercase italic tracking-[0.3em] text-[11px]">
        ...
        <div className="fixed bottom-10 left-1/2 -translate-x-1/2 z-[5000] animate-scale-up">
          <div className="bg-slate-900 text-white px-10 py-6 rounded-[2.5rem] shadow-2xl flex items-center gap-10 border border-white/10 backdrop-blur-md">
            <div className="flex items-center gap-3">
              <span className="w-8 h-8 bg-blue-500 rounded-xl flex items-center justify-center font-black text-xs">
            ...
            <button
              ...
              className="text-[10px] font-black uppercase text-slate-400 hover:text-white transition-colors ml-4"
            >
              Annuler
            </button>
          </div>
